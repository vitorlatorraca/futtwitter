...
function HashtagChip({ item, onClick }: { item: HashtagItem; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="px-3 py-1.5 rounded-full bg-x-search-bg hover:bg-x-border text-x-text-primary text-[14px] font-medium transition-colors"
    >
    ...
    <div className="animate-pulse">
      {[...Array(4)].map((_, i) => (
        <div key={i} className="px-4 py-3 border-b border-x-border">
          <div className="h-3.5 bg-x-border rounded w-24 mb-2" />
          <div className="h-4 bg-x-border rounded w-32 mb-1" />
          <div className="h-3 bg-x-border rounded w-16" />
        </div>
      ))}
    </div>
  );
...
function SkeletonHashtags() {
  return (
    <div className="flex gap-2 flex-wrap px-4 animate-pulse">
      {[...Array(8)].map((_, i) => (
        <div key={i} className="h-8 w-24 bg-x-border rounded-full" />
      ))}
    </div>
  );
}
    ...
    <div className="animate-pulse">
      {[...Array(3)].map((_, i) => (
        <div key={i} className="flex items-center gap-3 px-4 py-3">
          <div className="w-12 h-12 rounded-full bg-x-border" />
          <div className="flex-1">
            <div className="h-4 bg-x-border rounded w-24 mb-1" />
            <div className="h-3 bg-x-border rounded w-16" />
          </div>
...
export default function Explore() {
  ...
  const handleTrendingClick = useCallback(
    (item: TrendingItem) => {
      const name = item.title.startsWith("#") ? item.title : item.title;
      setSearchParams({ hashtag: name });
    },
    [setSearchParams]
  );
              ...
              {[...Array(5)].map((_, i) => (
                <div key={i} className="px-4 py-4 border-b border-x-border">
                  <div className="flex gap-3">
                    <div className="w-10 h-10 rounded-full bg-x-border" />
                    <div className="flex-1 space-y-2">
                      <div className="h-4 bg-x-border rounded w-1/3" />
                      <div className="h-4 bg-x-border rounded w-full" />
                    </div>
                  </div>
                </div>
              ))}
